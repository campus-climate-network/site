...
Narrow article layout: `mx-auto max-w-3xl px-(--spacing-container) section-hero stack stack-relaxed` (not the standard page-container width). Header: title (H1, `text-slate-900 leading-tight` — sanctioned exception), then a byline block with a 64px round author photo, name (`font-semibold text-slate-900`), date/categories (`text-sm text-slate-500`), and optional author bio. `<hr className="border-slate-200" />` before the `rounded-2xl` main image. Body typography is applied via arbitrary-variant selectors in the shared `PortableTextBody` wrapper ([`portable-text-body.tsx`](src/components/portable-text-body.tsx): `[&_h2]:text-2xl … [&_blockquote]:border-brand-primary/30 …`), also used by job postings — keep body element styles in that one component.

### Careers (`/hiring`, `/hiring/[slug]`)

Listing cards are the light card recipe made fully clickable with a stretched link on the title (`after:absolute after:inset-0`), so the title is the accessible link and nothing else inside is interactive. Anatomy: title (`text-lg font-semibold`, `group-hover:text-brand-primary`), neutral fact chips (`rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700` — type · location · pay), `line-clamp-3` summary, then a footer row with the deadline (`text-sm text-slate-500`) and a decorative "View role" text link. The posting page reuses the blog post's narrow article layout and the shared `PortableTextBody` wrapper; role facts sit in a tinted `<dl>` panel (`rounded-3xl bg-brand-secondary/[0.07]`, eyebrow `<dt>` + `font-medium text-slate-900` `<dd>`), and the Apply pill (primary, large, `ArrowUpRight`) sits directly under the facts panel and again in a centered closing light card (`px-6 py-12 text-center sm:px-12 sm:py-16`, H3-scale heading, same stacked composition as the gradient CTA panel); a closed role shows its notice under the lede instead.
